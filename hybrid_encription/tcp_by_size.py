...
def recv_by_size(sock, *, addition = None, down_a_line = True, return_type="string"):
    """
    ** returns a tuple with the message received and the size (so I could send size related errors) **
    ...
    if TCP_DEBUG and len(str_size) > 0 and isinstance(str_size, str):
        ...
        if addition is not None:
            print(addition, end='')
        print(f"Receive({str_size})>>>{data_to_print}")
...
def send_with_size(sock, data, down_a_line = True):
...
def recv_one_message(sock, return_type="string"):
    """
    Receive one message by two steps 4 bytes and all rest.
    """
